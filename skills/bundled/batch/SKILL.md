...
Once the plan is approved, spawn one background agent per work unit using the `Agent` tool. **All agents must use `subagent_type: "batch-worker"` and `run_in_background: true`.** Launch them all in a single message block so they run in parallel.
...
The `batch-worker` agent type automatically runs in an isolated git worktree — do not set isolation manually.
