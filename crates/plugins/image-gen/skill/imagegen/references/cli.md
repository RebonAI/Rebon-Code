...
Set a stable path to the skill CLI. `<skill-base-dir>` is the "Base directory for this skill" line at the top of the loaded skill:

```
export IMAGE_GEN="<skill-base-dir>/scripts/image_gen.py"
```
...
- Do not silently downgrade from CLI `gpt-image-2` or built-in `ImageGen` to CLI `gpt-image-1.5`; ask first unless the user explicitly requested `gpt-image-1.5`.
...
These are explicit CLI controls. They are not built-in `ImageGen` tool arguments.
