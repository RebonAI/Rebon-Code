---
name: "imagegen"
description: "Generate or edit raster images when the task benefits from AI-created bitmap visuals such as photos, illustrations, textures, sprites, mockups, or transparent-background cutouts. Use when Rebon should create a brand-new image, transform an existing image, or derive visual variants from references, and the output should be a bitmap asset rather than repo-native code or vector. Do not use when the task is better handled by editing existing SVG/vector/code-native assets, extending an established icon or logo system, or building the visual directly in HTML/CSS/canvas."
...
- **Default built-in tool mode (preferred):** built-in `ImageGen` tool for image generation, editing, and transparent-image requests. Does not require `OPENAI_API_KEY`.
...
- For transparent images, ask built-in `ImageGen` for a transparent background and preserve the generated alpha.
...
- In built-in tool mode, Rebon saves each generated image under its generated-images directory (default `~/.rebon/generated_images/<session>/`, configurable with `generatedImagesDir`). The `ImageGen` result names the exact saved path.
- Do not describe or rely on OS temp as the default built-in destination.
- Do not describe or rely on a destination-path argument (if any) on the built-in `ImageGen` tool. If a specific location is needed, generate first and then copy the selected output from the saved path the tool reported.
- Save-path precedence in built-in mode:
  1. If the user names a destination, move or copy the selected output there.
  2. If the image is meant for the current project, move or copy the final selected image into the workspace before finishing.
  3. If the image is only for preview or brainstorming, render it inline; the underlying file can remain at the default generated-images path.
...
- Built-in edit mode takes every target and reference image as a local file: pass up to 5 absolute paths in `referenced_image_paths`. That covers attached image files, project files, and images generated earlier in the session (use the saved path the earlier `ImageGen` result reported).
- If you have not seen a local image yet, inspect it with the `Read` tool first, then proceed with the built-in edit flow.
- An image that exists only inline in the conversation, with no file behind it, cannot be passed to `ImageGen`; ask the user to save or attach it as a file.
...
7. If you have not seen an edit target or reference image yet, inspect it with `Read` first, then pass its absolute path in `referenced_image_paths`.
...
11. For transparent-output requests, ask built-in `ImageGen` for a transparent background and preserve the generated alpha channel.
12. Inspect outputs and validate: subject, style, composition, text accuracy, and invariants/avoid items.
13. Iterate with a single targeted change, then re-check.
14. For preview-only work, render the image inline; the underlying file may remain at the default generated-images path.
15. For project-bound work, move or copy the selected artifact into the workspace and update any consuming code or references. Never leave a project-referenced asset only at the default generated-images path.
...
Ask built-in `ImageGen` for a genuinely transparent background and preserve its alpha.
...
- background-extraction — transparent background / clean cutout. Ask built-in `ImageGen` for actual transparency.
...
- Fallback-only execution notes such as `Quality:`, `Input fidelity:`, masks, output format, and output paths belong in the CLI path only. Do not treat them as built-in `ImageGen` tool arguments.
...
- For transparent images, ask built-in `ImageGen` for actual transparency and preserve its alpha.
...
- Do not ask the user for `OPENAI_API_KEY` when using the built-in `ImageGen` tool.
