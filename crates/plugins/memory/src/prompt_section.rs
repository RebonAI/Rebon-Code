...
//! seat.
//!
//! The plugin registers it at [`Rung::Memory`], which fixes where it lands
//! among the other sections: the rung is the rank, so the bytes a model
//! receives do not depend on when this plugin happened to load. Switch the
//! plugin off and the section is simply not there — nothing else in the
//! prompt shifts to cover for it.
//!
