...
pub struct Cell {
    ...
    /// The hyperlink target emitted around this cell by supporting backends.
    hyperlink: Option<CompactString>,
...
impl Cell {
    ...
    /// Gets the hyperlink target of the cell.
    #[must_use]
    pub fn hyperlink(&self) -> Option<&str> {
        self.hyperlink.as_deref()
    }

    /// Sets the hyperlink target emitted around this cell by supporting backends.
    pub fn set_hyperlink(&mut self, target: &str) -> &mut Self {
        self.hyperlink = Some(CompactString::new(target));
        self
    }

    /// Clears the hyperlink target of the cell.
